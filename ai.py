...
class ConvNet(nn.Module):
    def __init__(self, input_dim, hidden_dim, output_dim):
        ...
        self.layer1 = torch.nn.Sequential(
            torch.nn.Conv2d(4, input_dim * input_dim, kernel_size=5, stride=1, padding=2),
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(kernel_size=5, stride=1),
            torch.nn.Dropout(p=0.05))
        self.layer2 = torch.nn.Sequential(
            torch.nn.Conv2d(input_dim * input_dim, output_dim, kernel_size=5, stride=1, padding=2),
            torch.nn.ReLU(),
            torch.nn.MaxPool2d(kernel_size=5, stride=1),
            torch.nn.Dropout(p=0.1))
        self.fc2 = torch.nn.Linear(output_dim, 1, bias=True)
...
class GomokuAI:
    ...
    def train_short_memory(self, state, action, reward, scores, next_state, next_scores, done):
        ...
        state = torch.cat((scores_tensor, state), dim=1)
        state_flattened = state.view(-1)  # flattened state
        next_state = torch.tensor(next_state, dtype=torch.float).unsqueeze(0)
        next_state = torch.cat((next_scores_tensor, next_state), dim=1)
        ...
        q_pred = self.model(state)
        print(f"q pred: {q_pred}")
        q_next = next_state
        print(f"q next: {q_next}")
        q_target = q_pred.clone()
        q_new = q_pred + self.learning_rate * (reward + self.gamma * torch.max(self.model(q_next) - q_pred))
        print(f"torch max: {torch.max(self.model(q_next)-q_pred)}")
        print(f"q new: {q_new}")
